...
ตั้งค่าเปลี่ยนเป็น UX แบบหน้ารวมและหน้าตั้งค่าย่อยจริง เพื่อให้สอดคล้องกับภาพตัวอย่างและลดรายการยาวในหน้าเดียว หน้ารวมมีการ์ดที่กดได้จริง 4 รายการ แต่ละรายการใช้ไอคอนเวกเตอร์ ชื่อ คำอธิบายสั้น และ chevron:

1. `การปกป้อง` — โปรไฟล์ปัจจุบันและการตั้งค่าตรวจจับเฉพาะโปรไฟล์
2. `การแจ้งเตือนและความปลอดภัย` — Telegram, SMS สำรอง และ pairing
3. `ความต่อเนื่องของระบบ` — สิทธิ์ที่จำเป็น และคำแนะนำแบตเตอรี่/OEM keep-alive
4. `การวินิจฉัยขั้นสูง` — diagnostics, sensor configuration และ audio ที่มีอยู่จริง

การกดการ์ดเปิดหน้าตั้งค่าย่อยที่มี control เดิมจริง ไม่ใช่ overview ปลอม หน้าย่อยมีหัวหน้าสีน้ำเงิน ชื่อหน้า ปุ่มย้อนกลับที่เข้าถึงได้ และกลับมายังหน้ารวมเมื่อกดปุ่มย้อนกลับของระบบ การนำทางย่อยเป็น UI-only state ที่บันทึกคืนได้ (`rememberSaveable`) ไม่เพิ่ม destination ระดับแอป ไม่เพิ่ม deep link และไม่แก้ `ProtectionDestination` สามแท็บหลัก
...
4. สร้างหน้ารวมตั้งค่าและ navigation ภายใน พร้อม test ปุ่มย้อนกลับและ state ที่บันทึกคืนได้
5. ปรับหน้าตั้งค่า `การปกป้อง`
6. ปรับหน้าตั้งค่า `การแจ้งเตือนและความปลอดภัย`
7. ปรับหน้าตั้งค่า `ความต่อเนื่องของระบบ` และ `การวินิจฉัยขั้นสูง`
8. ตรวจ contract, accessibility, build และ device acceptance
...
รับงานเมื่อทั้ง 3 หน้าจอในเส้นทางจริงและหน้าตั้งค่าย่อยทั้ง 4 เป็นไปตามดีไซน์นี้ ไม่มีอิโมจิที่ผู้ใช้เห็นในเส้นทางนั้น การนำทางและ control ยังเข้าถึงได้ พฤติกรรมความปลอดภัยที่มีอยู่ยังเหมือนเดิม focused tests รวมถึง full host tests, Android-test compilation และ APK build ผ่าน และมีการตรวจอุปกรณ์ใหม่ก่อนอ้างว่าได้ผลด้านภาพหรือ accessibility บนอุปกรณ์จริง

## 8. สิ่งที่ไม่ทำ

- ไม่เพิ่มฟีเจอร์ dependency destination ระดับแอปใหม่ deep link filter ปลอม หรือออกแบบหน้าทางเก่าใหม่
