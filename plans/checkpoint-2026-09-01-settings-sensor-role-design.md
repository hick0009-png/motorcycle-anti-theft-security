...
> อัปเดตล่าสุด: 2026-09-02 — T1-T8, T10, T11 เสร็จแล้ว เหลือ T9, T12-T16 ดูหัวข้อ "ความคืบหน้า"

## Resume point

- **Worktree:** `D:\security` (main worktree)
- **Branch:** `feature/motorcycle-guard-protection`
- **HEAD:** `7d74d89` — `feat(settings): summarise what the selected use is actually set up to run` (2026-09-02)
- **สถานะการทำงาน:** T1-T8, T10, T11 ทำเสร็จแล้ว (ดู "ความคืบหน้า" ด้านล่าง)
...
| T6 กันตั้ง "หลัก" ให้เซ็นเซอร์ที่เครื่องไม่มี | ✅ เสร็จ | `b596adc` |
| T7 หมายเหตุความจริงโหมดประตู/ไฟเลี้ยง | ✅ เสร็จ | `ae84776` |
| T8 สรุปหัวการ์ด หลัก/ประกอบ/ปิด | ✅ เสร็จ | `7d74d89` |
...
T6 → T7 → T8               ✅ เสร็จ (b596adc, ae84776, 7d74d89)
T14 + T16                  ← ก้อนถัดไป ⚠️ คอมมิตเดียวกัน + ประกาศ LOCATION = เจ้าภาพเฝ้ารถ
...
2. **ก้อนถัดไปตามลำดับที่ตกลง: T14 + T16** (เจ้าของสั่ง "ตามลำดับ" เมื่อ 2026-09-02)
   ⚠️ ต้องอยู่คอมมิตเดียวกัน และประกาศ `LOCATION = เจ้าภาพของโหมดเฝ้ารถ` ไปพร้อมกัน
   มิฉะนั้นการเตือน "รถถูกเคลื่อนย้าย" จะเงียบทั้งหมดโดยไม่มี error — เขียนเทสต์ล็อกพฤติกรรมก่อนแตะ
   ถ้าจะสลับไปก้อนอื่น ต้องถามก่อนลงมือ
3. **T12 ควรเริ่มขนานตั้งแต่ตอนนี้** เพราะต้องรอข้อมูลดริฟท์ข้ามคืน ไม่ใช่เวลาเขียนโค้ด
   และ T13 ตัดสินใจไม่ได้เลยจนกว่าจะได้ตัวเลข
...
- host: 1003 ตัวผ่าน 0 fail
- instrumented บน INE-LX2: `SensorRecommendationUiTest` 27/27,
  `SensorLockUiTest` 10/10, `ProfileDeviceSupportUiTest` 4/4
- ติดตั้งบิลด์จาก `7d74d89` ลงเครื่องแล้ว

### ⚠️ อาการหลอกตอนรันเทสต์บนเครื่อง

ถ้าเทสต์แดงทั้งคลาสด้วย `No compose hierarchies found in the app`
**ให้ดูจอเครื่องก่อนไล่โค้ด** — เกิดจากเครื่องหลับ/ล็อกหน้าจอ ไม่ใช่โค้ดพัง
แก้: `adb shell input keyevent KEYCODE_WAKEUP` แล้วปัดปลดล็อก
กันไว้: `adb shell svc power stayon usb`

## ข้อพิสูจน์บนเครื่องที่ยังค้าง — เจ้าของต้องรันเอง

ผลของ T7 มาจากการอ่านโค้ดล้วน ๆ ยังไม่มีหลักฐานจากของจริง

1. โหมดประตู → ตั้งไจโรสโคป + เวกเตอร์การหมุนทั้งสามเป็น "ปิด" → Arm → เปิดประตูจริง
   → **ต้องยังได้ "🚪 ตรวจพบประตูเปิด"** ถ้าไม่ได้ = อ่านโค้ดผิด หมายเหตุที่ใส่ไปเป็นคำโกหก
   ต้องกลับมาทบทวน T7 ทันที
2. โหมดไฟเลี้ยง → ลดบทบาทเซ็นเซอร์แสงเป็น "ประกอบ" → การเฝ้าไฟยืนยันต้องยังทำงาน (ข้อค้นพบ #12)

ไม่ได้รันให้เพราะทั้งสองข้อยิงข้อความจริงไปหาเจ้าของ
