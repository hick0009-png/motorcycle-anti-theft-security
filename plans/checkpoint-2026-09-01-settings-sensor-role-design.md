...
> อัปเดตล่าสุด: 2026-09-02 — T1-T5, T10, T11 เสร็จแล้ว ดูหัวข้อ "ความคืบหน้า"

## Resume point

- **Worktree:** `D:\security` (main worktree)
- **Branch:** `feature/motorcycle-guard-protection`
- **HEAD:** `8b8ed86` — `feat(settings): compare what all three roles would do before one is chosen` (2026-09-02)
- **สถานะการทำงาน:** T1, T2, T10, T11, T3, T4, T5 ทำเสร็จแล้ว (ดู "ความคืบหน้า" ด้านล่าง)
...
| T10 สถานะฮาร์ดแวร์จริงขึ้นหน้าจอ | ✅ เสร็จ | `b376d1b` |
| T11 กันเลือกโหมดที่เครื่องรองรับไม่ได้ | ✅ เสร็จ | `16c6e6d` |
| T3 ป้าย ⭐ แนะนำ + ปุ่มคืนค่า | ✅ เสร็จ | `9d0177a` |
| T4 บรรทัด "ช่วยตรวจจับ" ต่อโปรไฟล์ | ✅ เสร็จ | `7e8dbbd` |
| T5 แผงเทียบผล 3 บทบาท | ✅ เสร็จ | `8b8ed86` |
| T9 เดินสายไป profile overrides | ⏸ ยังไม่ทำ (เฟส 2 ของส่วนที่ 1 ข้อ 7) | — |

### ⚠️ สามจุดที่ "เอกสารออกแบบส่วนที่ 2" เขียนผิด — พิสูจน์จากโค้ดแล้วตอนทำ T4

ห้ามกลับไปเขียนตามเอกสารเดิม เทสต์ตรึงไว้แล้วทั้งสามข้อ

| # | เอกสารเขียนว่า | ความจริงในโค้ด | ที่ |
|---|---|---|---|
| ก | แสง × เฝ้ารถ "ตั้งเป็นหลัก = เปิดเหตุการณ์ได้เอง" | **แสงไม่เคยเปิดเหตุการณ์เองเลย** ไม่ว่าบทบาทใด — เก็บเป็น precursor แล้ว `return Ignored` บทบาทมีผลแค่ว่าการสั่นที่ตามมาจะเปิดได้หรือไม่ | `IncidentEngine.kt:67-70` |
| ข | "ยิ่งเพิ่มตัวหลัก ยิ่ง Arm ช้า" (ข้อ 1.2.2) | `hasReadyPrimary` เป็น **`any` ไม่ใช่ `all`** — ต้องมีตัวหลักพร้อมหนึ่งตัวก็พอ เพิ่มตัวหลักไม่ทำให้ช้าลง | `ProtectionCoordinator.kt:1311` |
| ค | (ไม่ได้เขียนไว้) | **โหมดไฟเลี้ยงลงทะเบียน `TYPE_LIGHT` เองโดยตรง** ไม่ผ่าน config — ข้อค้นพบ #4 ฝาแฝดฝั่งไฟเลี้ยง ลดบทบาทแสงลงไม่ทำให้การเฝ้าไฟยืนยันหยุด | `AndroidProtectionRuntime.kt:998` |

ข้อ ค เพิ่มเป็น **ข้อค้นพบ #12** ใส่ `caveatTh` ไว้แล้วรอ T7 เอาไปแสดง
(ประตู × แกนหมุน 3 ตัว และ ไฟเลี้ยง × แสง — สองที่นี้เท่านั้นที่บทบาทไม่คุมการตรวจจับจริง)
...
```
T1 → T2                    ✅ เสร็จ (e065013)
T10 → T11                  ✅ เสร็จ (b376d1b, 16c6e6d)
T3 → T4 → T5               ✅ เสร็จ (9d0177a, 7e8dbbd, 8b8ed86)
T6 → T7 → T8               ← ก้อนถัดไป · กับดักฮาร์ดแวร์ · ความจริงโหมดประตู · สรุปภาพรวม
...
2. **ก้อนถัดไปตามลำดับที่ตกลง: T6 → T7 → T8** (เจ้าของสั่ง "ตามลำดับ" เมื่อ 2026-09-02)
   ถ้าจะสลับไปก้อนอื่น ต้องถามก่อนลงมือ
3. T6 ต่อยอดจาก T10/T11 ได้ทันที: `SensorAvailabilityUiModel` ถึง UI แล้ว
   เหลือแค่กันไม่ให้ตั้ง "หลัก" กับ source ที่ `availability == MISSING`
   (ของจริง: `SensorCapabilityController.kt:96` ปฏิเสธทั้งชุด ส่วน "ประกอบ" แค่ degraded)
4. T7 มีเนื้อหาพร้อมแล้วใน `SensorContribution.caveatTh` — เหลือแค่เอามาแสดงในแถว/แผง
...
## เทสต์ ณ HEAD ปัจจุบัน

- host: 995 ตัวผ่าน 0 fail
- instrumented บน INE-LX2: `SensorRecommendationUiTest` 15/15,
  `SensorLockUiTest` 10/10, `ProfileDeviceSupportUiTest` 4/4
- ติดตั้งบิลด์จาก `8b8ed86` ลงเครื่องแล้ว
